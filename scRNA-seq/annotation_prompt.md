...
# 任务 (Task)
请根据我提供的单细胞集群（Cluster）的 FindMarkers 差异表达结果及样本背景信息，对各个 Cluster 进行精确的细胞类型推断，并严格采用“两级身份制（谱系+状态）”输出结论。
...
1. **核心指标综合评估**：
   - **特异性（核心）**：绝对不能仅看 `pct.1`，必须结合 `pct.2`。优先选取 `pct.1` 高且 `pct.2` 显著较低的基因（特异性高）；
   - **显著性**：结合 `avg_log2FC`（表达倍数变化）和 `p_val_adj`（统计显著性）确认差异的可靠性。
2. **两级身份鉴定与 Marker 分离**：
   - **Level 1（细胞大类/谱系）**：寻找并确认决定细胞基础谱系的经典 lineage marker（如判断是 Macrophage 还是 T cell）；
...
3. **双细胞污染（Doublet）排查**：
   - 若同一 Cluster 强烈共表达两套来自不同独立谱系的特异性 Marker，应高度怀疑 Doublet；
   - 需结合表达比例和特异性排除干扰：若是少数广谱基因、吞噬作用或环境 RNA 污染，则不应轻易判定为 Doublet。

# 输出格式要求 (Output Format)
为了保证大模型推理的严密性（Chain of Thought），请**先对每个 Cluster 进行详细解析，最后再输出汇总表格**。
...
2. **推断依据与 Marker 解析**：
   - **Level 1 依据**：列出支持基础谱系的关键 Marker 及其数据表现（如 pct.1 与 pct.2 的差异比例）；
